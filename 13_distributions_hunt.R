...
###  hunter harvested deer, revised for multiple deer of each sex/ageclass
###
###   d_fit_hunt
###
#######################################################################
...
dFOIhunt <- nimble::nimbleFunction( # nolint
    ...
    # start the loop through individual case types
    sumllik <- 0
        for (i in 1:n_samples) {
            ...
            if (sex[i] == 0) { # age loops for females
                ...
                p <- 1 - exp(-lik_foi)
                lik_temp <- dbinom(test_status[i], 1, p, log = TRUE)
...
nimble::registerDistributions(list(
    dFOIhunt = list(
        BUGSdist = "dFOIhunt(test_status,n_cases,n_samples,a,sex,age2date,f_age_foi,m_age_foi,age_lookup_f,age_lookup_m,f_period_foi,m_period_foi,period_lookup_foi,space,sect)",
        types = c(
            "value = integer(0)",
            "test_status = double(1)",
            "n_cases = double(1)",
...
# starttime <- Sys.time()
# test <- dFOIhunt(
#     test_status = d_fit_hunt$teststatus,
