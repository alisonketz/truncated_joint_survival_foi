...
d_huntseason <- read_xlsx(paste0(filepath,
                                 "Hunting_SeasonDates.xlsx"),
                                 1)
...
# d_season <- d_season %>% filter(Year > 2016)

startdate <- study_origin
...
# d_season <- d_season[d_season$Year>2016,]
# season_ng_start <- c()
# season_ng_end <- c()
...
# # # nonineday <- c(2017:2021)[which(!(c(2017:2021) %in% temp$Year))]
# # temp2 <- d_season %>% 
...
#           temp$OpenDate[temp$Year == (i + 2016)]) / weeks(1))

#     season_gun_end[i] <-  ceiling(interval(study_origin,
#           temp$CloseDate[temp$Year == (i + 2016)]) / weeks(1)) 
# }
...
end_dates <- paste(2018:2022,"05","14",sep="-")
...
d_fit_season <- matrix(NA, nrow = n_year_collar, ncol = 8)
d_fit_season[1, ]  <-  c( 1,
                          startng[1] - 1,
                          startng[1],
                          startgun[1],
