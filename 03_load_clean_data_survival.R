...
# database <- mdb.get('datafiles/SWDPPdeerDB.mdb')

d_cap <- mdb.get('datafiles/SWDPPdeerDB.mdb',tables= "Adult_Capture")
d_fawncap <- mdb.get('datafiles/SWDPPdeerDB.mdb',tables= "Fawn Capture")
d_mort <- mdb.get('datafiles/SWDPPdeerDB.mdb',tables= "Mortalities")
d_cens <- mdb.get('datafiles/SWDPPdeerDB.mdb',tables= "Censor")
d_cwd <- mdb.get('datafiles/SWDPPdeerDB.mdb',tables= "RAMALT")
d_post_cwd <- mdb.get('datafiles/SWDPPdeerDB.mdb',tables= "Postmortem_CWD")
d_tooth <- mdb.get('datafiles/SWDPPdeerDB.mdb',tables= "Tooth")
d_age <- read.csv('datafiles/Age_Uncertain.csv')
...
### There were 47 of these (17 morts, 30 cens), 
### while they may be tagged
### these should not be included in the analysis
###
#########################################################
...
### to obtain age at capture for collared deer
###
#########################################################################
...
#########################################################################
###
### There are 12 individuals that had teeth pulled but no age
### was determined, in 2 cases, the tooth broke so bad sample
### Imputing these ages based on the
### stable age distribution of collared deer
###
#########################################################################
...
### discretized for weekly or monthly
###
intvl_step_yr_weekly <- 52
intvl_step_yr_monthly <- 12
...
for (i in 1:n_cap){
  ...
  if(month(ymd(d_cap$date[i])) < 6 ){
    ...
    d_cap$agemonth[i] <- month(ymd(d_cap$date[i])) - 6 + floor(d_cap$ageatcap[i]) * intvl_step_yr_monthly
  } 
}
...
names(df_use_fawncap)[2:ncol(df_use_fawncap)] <-
          paste0(names(df_use_fawncap)[2:ncol(df_use_fawncap)], "_cap")
...
low_af_pos_recap <- unique(c(dup$lowtag[dup$cwdstatus3_cap > 0],
                             dup$lowtag[dup$cwdstatus4_cap > 0]))

low_af_neg <- low_af_neg[!(low_af_neg %in% low_af_pos_recap)]

### For the fawns that were recaptured, 
### include the recapture date in the recap and recap2 columns
...
### for the recaptured fawns that were positive,
### we have to include that info in the cwdstatus col
...
df_cap$cwd_cap <- c()
df_cap$cwd_cap[df_cap$year_cap == 2017] <-
          df_cap$cwdstatus1_cap[df_cap$year_cap == 2017]
df_cap$cwd_cap[df_cap$year_cap == 2018] <-
          df_cap$cwdstatus2_cap[df_cap$year_cap == 2018]
df_cap$cwd_cap[df_cap$year_cap == 2019] <-
          df_cap$cwdstatus3_cap[df_cap$year_cap == 2019]
df_cap$cwd_cap[df_cap$year_cap == 2020] <-
          df_cap$cwdstatus4_cap[df_cap$year_cap == 2020]
df_cap$recap_cwd <- rep(NA, nrow(df_cap))

#2017 recaptured deer that were initially captured in 2017
colindx_recap1 <- year(df_cap$recapdate_cap[which(!is.na(df_cap$recapdate_cap) &
                                          df_cap$year_cap == 2017)]) - 2017

temp <- df_cap[which(!is.na(df_cap$recapdate_cap) &
              df_cap$year_cap == 2017), 17:19]
yr1recap_cwd <- c()
for(i in 1:nrow(temp)){
  yr1recap_cwd[i] <- temp[i, colindx_recap1[i]]
}
df_cap$recap_cwd[which(!is.na(df_cap$recapdate_cap) &
              df_cap$year_cap == 2017)] <- yr1recap_cwd

#2018 recaptured deer that were initially captured in 2018
colindx_recap2 <- year(df_cap$recapdate_cap[which(!is.na(df_cap$recapdate_cap) &
            df_cap$year_cap == 2018)]) - 2017
...
df_cap$recap_cwd[which(!is.na(df_cap$recapdate_cap) &
                  df_cap$year_cap == 2018)] <- yr2recap_cwd

#2019 recaptured deer that were initially captured in 2019
colindx_recap3 <- year(df_cap$recapdate_cap[which(!is.na(df_cap$recapdate_cap) &
                                            df_cap$year_cap == 2019)]) - 2017
temp <- df_cap[which(!is.na(df_cap$recapdate_cap) &
                    df_cap$year_cap == 2019), 17:19]
yr3recap_cwd <- c()
for(i in 1:nrow(temp)){
...
##########################################################################
###
### Removing mortalities & censors for deer that die (or censored)
### after May 14, 2022,
### the end of the study
### these are right censors
###
#########################################################################

low_died_after <- unique(d_mort$lowtag[which(d_mort$estmortdate > "2022-05-14")],
                  d_mort$lowtag[which(d_mort$mortalertdate > "2022-05-14")],
                  d_mort$lowtag[which(d_mort$collarfound > "2022-05-14")])

d_mort <- d_mort[!(d_mort$lowtag %in% low_died_after),]

rm_cens_poststudy <- which(!is.na(d_cens$censordate) &
                           d_cens$censordate > "2022-05-14")

low_died_after <- d_cens$lowtag[rm_cens_poststudy]
d_cens <- d_cens[-rm_cens_poststudy,]
...
##########################################################################
###
### Overall survival  defaults to censored even
...
class(d_mort$cause1wt) <- "numeric"
class(d_mort$cause2wt) <- "numeric"
class(d_mort$cause3wt) <- "numeric"
