...
# cwd_df$birthweek <- (interval(study_origin,
#                             cwd_df$birth_date) %/% weeks(1))
...
d_fit_hunt <- cwd_df %>% 
              ...
              group_by(teststatus, ageweeks, sex, ew) %>%
              summarise(n_cases = n(), .groups = 'drop')

d_fit_hunt_neg <- d_fit_hunt[d_fit_hunt$teststatus == 0, ]
d_fit_hunt_pos <- d_fit_hunt[d_fit_hunt$teststatus == 1, ]
