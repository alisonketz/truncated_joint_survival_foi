  ...
  run = function(age_lookup_f = double(1),
   ...
   # infection probability all ages all years 
    for(k in 1:n_study_area) {
        for (a in 1:n_agef) {
            p_inf[k, 1, a] <- 
                1 - exp(-sum(gam[k,1,yr_start_age[a]:(yr_start_age[a] + 51)]))
        }
        for (a in 1:n_agem) {
            p_inf[k, 2, a] <- 
                1 - exp(-sum(gam[k,2,yr_start_age[a]:(yr_start_age[a] + 51)]))
        }
    }

    returnType(double(3))
    return(p_inf[1:n_study_area, 1:n_sex, 1:n_agef])
  })
...
#                         nT_period = nT_period,
#                         period_lookup_foi_study = period_lookup_foi_study[1:nT_period],
#                         n_year = n_year,
#                         n_sex = n_sex,
...
calc_infect_prob_hunt <- nimbleFunction(
  run = function(age_lookup_f = double(1),
                 ...
                 nT_age_surv_aah_f = double(0),
                 nT_age_surv_aah_m = double(0),
                 calibration_transition = double(0)) {
